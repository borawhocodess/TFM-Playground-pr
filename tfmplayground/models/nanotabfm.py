# Vendored from https://github.com/google-research/tabfm (src/pytorch/model.py),
...
class CellEmbedder(nn.Module):
  def __init__(self, embed_dim, max_classes, feature_group_size=3, num_freq=32,
               is_classifier=True, fourier_sigma=1.0):
    super().__init__()
    self.embed_dim = embed_dim
    self.fgs = feature_group_size
    self.is_classifier = is_classifier
    in_dim = feature_group_size
    self.fourier_frequencies = nn.Parameter(torch.randn(in_dim, num_freq) * fourier_sigma)
    self.fourier_frequencies_cat = nn.Parameter(torch.randn(in_dim, num_freq) * fourier_sigma)
...
class ICLearning(nn.Module):
  def __init__(self, d_model, num_blocks, nhead, max_classes, dim_ff,
               decoder_hidden, is_classifier=True, num_outputs=1):
    super().__init__()
    ...
    else:  # MLP y-encode the scalar target; decode to num_outputs (regression buckets)
      self.y_encoder = MLP(1, [decoder_hidden], d_model)
      self.decoder = MLP(d_model, [decoder_hidden], num_outputs)
...
class TabFMModel(TabularFoundationModel):
  def __init__(self, *, embed_dim=8, max_classes=3, col_num_blocks=2,
    ...
    self.output_kinds = {
        problem: "class_logits" if is_classifier else ("scalar" if num_outputs == 1 else "bar_logits")
    }
    self.num_outputs = max_classes if is_classifier else num_outputs
    ...
    self.cell_embedder = CellEmbedder(embed_dim, max_classes, feature_group_size,
                                      num_freq, is_classifier, fourier_sigma)
    ...
    self.icl_predictor = ICLearning(icl_dim, icl_num_blocks, icl_nhead, max_classes,
                                    icl_dim * ff_factor,
                                    decoder_hidden or icl_dim * 2, is_classifier, num_outputs)

  def _forward_full(self, x, y, train_size, cat_mask=None, d=None):
  ...
  def forward(self, X_train: torch.Tensor, y_train: torch.Tensor, X_test: torch.Tensor) -> torch.Tensor:
    ...
    y_pad = torch.zeros(b, X_test.shape[1], device=y_train.device, dtype=y_train.dtype)
    y = torch.cat([y_train, y_pad], dim=1)
    train_size = torch.full((b,), n_train, device=x.device, dtype=torch.long)
    out = self._forward_full(x, y, train_size)
    return out[:, n_train:].contiguous()
